...
# in DCMW we use not random network
class BCN(tf.keras.Model):

    def __init__(self,
                 input_dim: int, 
                 output_dim:int,
                 controller_dime:int=64,
        ...
        super(BCN,self).__init__(name=name)
        ...
        self.network_forget_rate=1/12 #random network forgeting data in time, see testin random network
        ...
        #Creating Random Ntwork Wieghts -- using numpy becuse its faster than tf
        ...
        for row in range(self.netsize):
            if (row%3==0): 
                for i in range(self.degree):
                    self.randomnetwork[row,row-i]=-1
            else:
                for i in range(degree):
                    self.randomnetwork[row,row-i]=1/2
    ...
    def call(self, x: Union[np.ndarray, tf.Tensor]) -> tf.Tensor:
        """ Unstack the input, run through the DCNW and return the stacked output. """
