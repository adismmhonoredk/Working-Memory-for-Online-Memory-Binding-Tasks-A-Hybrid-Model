...
class DCWM(tf.keras.Model):
    ...
    def __init__(self,
        ...
        self.network_forget_rate=1/3 #random network forgeting data in time, see testin random network
        ...
        self.dense3 = Dense(
                         self.interface_dim, # last_layer_of_dense_dim
                         activation='sigmoid',
                         name='dcwm_controller')
        
        # Derived Output and interface vector from controller last layer (dense_output)       
        temp=np.zeros((self.interface_dim, self.output_dim))
        temp_dim=self.interface_dim
        ...
        self.W_output = tf.Variable( #[last_layer_of_dense_dim,output_dim]
            tf.Variable(temp,dtype=tf.float32),
            name='dcwm_net_output_weights'
        )

        temp=np.zeros((self.interface_dim, self.interface_dim))
        temp_dim=self.interface_dim
        for i in range(temp_dim):
    ...
    def step(self, x: tf.Tensor) -> tf.Tensor:
        ...
        # Update controller
        y_out = self.dense3(self.dense2(self.dense1(x_in)))
